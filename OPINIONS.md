...
He treats human time and attention as too scarce for mechanical session ops, and warns that manual "/compact" is often an expensive same-model summarization call that destroys cache hits on the next turn, so default auto-compaction was his baseline so humans can spend attention on what to do next rather than micro-optimizing windows; once cheap specialized judgment can classify whether the session sits at a task boundary where future actions are unlikely to need unpersisted prior detail, selective compaction advice becomes worth getting, optimizing for precision while the window is small and shifting toward recall as it fills because missed compaction eventually becomes forced compaction anyway; he still rejects "instant compaction" that only drops some tool calls while keeping user and assistant messages forever, because the summary never shrinks, long tasks still exhaust the window, and mutating history before compacting turns a cheap fully-cached session into a 10x-40x cache-miss prompt.
