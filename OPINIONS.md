...
Kun judges coding agents by whether they complete valuable work in messy real codebases, not by toy demos, impressive screenshots, or code-volume statistics, and it bothers him when people jump on cool demos without careful analysis or basic scientific rigor in proving value.
...
He believes AI amplifies competence and judgment: weak taste and weak requirements produce more slop faster, while the residual human job is the subjective feel that agents miss unless asked; "slop" is a moving bar where anything one-shotable by AI becomes low-effort and worthless once millions can do it, including correctly implemented products nobody wants, not only buggy code.
Because of that, fully automated software factories fail by definition, working systems put humans at the center to amplify craftsmanship rather than replace it, and unsupervised agent runs accumulate work that "does not feel right," so human taste remains required throughout building rather than only at the start, and the leverage bottleneck moves toward taste, mission, focus, and quality of thought.
...
He thinks the top-level orchestrator needs sound judgment for carrying user rationales, prioritizing work, resolving ambiguity, and handling escalations, but putting peak-intelligence fable-class models on mundane orchestration steps such as kicking off the next finished task makes cost untenable; he prefers a cheap fast orchestrator that is heavily steered to escalate ambiguous decisions to premium models without lowering overall system quality.
...
He treats human time and attention as too scarce for mechanical session ops, and warns that manual "/compact" is often an expensive same-model summarization call that destroys cache hits on the next turn, so default auto-compaction was his baseline; once cheap specialized judgment can classify task-boundary safety, selective compaction advice becomes worth getting (precision while the window is small, recall as it fills), and he expects harnesses to eventually ship that advice built-in so end users stop micro-managing windows.
He treats idle prompt-cache expiry as an operational trap: returning to a long idle session and resuming or "/compact"ing after the cache expired can burn a full uncached large-window request, so he prefers compacting before walking away, or else starting a fresh session and pointing the agent at the prior transcript most harnesses already save, while still rejecting "instant compaction" that only drops some tool calls because the summary never shrinks and mutating history before compacting turns a cheap fully-cached session into a 10x-40x cache-miss prompt.
...
He treats voice input as especially useful for agents and a poor fit for polished writing: talking is autoregressive, so early wording corners later framing, and pauses or corrections feel high-stakes with humans or prose, but agents tolerate messy rambling, mid-stream rephrasing, and spoken thinking; he advises speaking the reasoning process and correcting often so the agent sees why, while treating typing as better for polished content and precise code because it hides thinking tokens and shows mostly the final what, and still not as a full keyboard replacement for tight terminal, tmux, editor, and command workflows.
...
He sees Claude as pleasant for interactive work, while GPT or Codex can be better for non-interactive background execution, bug finding, and skill invocation, though he finds GPT judgment often off when asked to supervise complex work rather than follow exact instructions; some ultra-cheap multi-turn models (a "slow mode" opposite of fast-mode upcharges) are extreme ROI as overnight or well-understood bug-fix workhorses even when turn count is high, yet unusable as interactive orchestrators because events pile up faster than they finish, so he reserves them for clear tasks and keeps judgment models for ambiguous or creative planning.
...
He values agent-agnostic and vendor-agnostic workflows as practical resilience against provider outages, quota limits, model-specific product failures, and a market where OpenAI, Anthropic, and other labs may each be best for different parts of the work, and as a shipping-speed advantage: a harness-agnostic orchestrator can integrate a new specialized engine the same day it ships, while a first-party stack locked to its own models cannot; he notes labs recreating orchestration features often arrive months behind and still miss that vendor-agnostic piece.
...
He evaluates model upgrades by daily work usefulness, not version-number vibes or benchmarks alone: perceivable improvement, appropriately justified verbosity rather than fluffy filler, behavioral reliability that avoids jump-to-wrong then admit-and-fix loops, cost-quality fit, quota packaging, orchestration speed, and fewer unnecessary clarification turns matter as much as peak scores, and widespread rollbacks to older versions across labs are a real signal that a release failed that test; he does not assume a newer release is better until he tries it himself and keeps the version that is faster, more efficient, or more straight to the point for his work.
...
He treats reasoning-effort knobs as a thinking-token ceiling, not a floor: post-training penalizes long traces more strongly at low effort and more weakly at high effort, so high effort only allows more thinking when needed rather than forcing it on every request; he mostly uses medium for well-defined implementation after planning has resolved ambiguity and xhigh for planning or investigative work, generally avoids max modes that require thinking tokens on every turn, notes that many coding tasks benefit more from additional tool calls than from longer thinking traces while creative work still needs higher thinking, treats mid-session effort switches as expensive because they often break prompt caching unless the harness preserves it, and keeps the knob user-set because models do not know a user's cost-versus-quality tradeoff for a given task.
He maps model choice onto workflow buckets rather than one winner: interactive orchestrators that are fast, pleasant, and judgment-good enough to steer a crew; scarce premium-intelligence models for highly ambiguous or creative work and for escalations when crews argue or review loops spiral; planners trusted for specs and complex bug investigation; efficient implementers for well-defined specs, often at medium effort; and trivial fixers for one-liner or already-defined changes, treating a model that spans most buckets as a major subscription advantage, still reaching for higher-ceiling creative models when he wants ambiguous prototyping to surprise him, and sometimes preferring a strong implementer-class model as a high-volume adversarial reviewer when premium models are too expensive for that volume.
For constrained choice problems where options are predefined but the pick is hard, including user-defined freeform routing categories that no shared traditional classifier can cover, he has deployed specialized non-generative decision engines in production for near-real-time model routing and similar triage, including as a drop-in router inside chief-of-staff orchestrators that assume substantial work goes to subagents, seeing frontier-model agreement at far lower cost and latency than an orchestrator LLM that reads dispatch context and autoregresses the choice, sometimes cheap enough that heavy usage barely moves metered spend, while rejecting them as replacements for generative LLMs on open-ended tasks.
...
He expects frontier-model usage patterns to get heavier as loops, agent teams, and chief-of-staff workflows become normal, so labs should offer higher tiers, clearer quota tradeoffs, and distinct slower cheaper background modes instead of forcing power users into cap loops or interactive-only stacks.
...
Evidence: https://x.com/kunchenguid/status/2103277022235766963, https://x.com/kunchenguid/status/2103325882752598078
...
He is skeptical of lab strategies that over-optimize for peak text or coding intelligence while falling behind on efficiency, transparent capacity, and multimodal product coverage, and he reads some multi-tier model naming and SKU launches as competitive perception engineering: labs can invent mid tiers or rename ladders to look like peer answers to rivals, buy time while a real frontier catch-up ships, then drop the artificial middle once it has done its job.
...
He gives credit when a lab listens to users, reverts bad product decisions quickly, and ships practical workflow affordances that reduce babysitting rather than only chasing model spectacle, including adopting shared agent conventions such as AGENTS.md when a strong model is held back by a proprietary-only product approach.
...
Evidence: https://x.com/kunchenguid/status/2103730633780674626, https://x.com/kunchenguid/status/2098854862788440308
...
He used to weight speed to market heavily, but recent AI product and model releases changed his mind: because AI use is mostly private, network effects are hard to create, users can switch to something better or cheaper, and an early user base does not lock in a moat the way social contact graphs do, so building the right thing beats being first and later competitors can still take most of the share.
He thinks product-layer moats need genuine network effects such as multiplayer or marketplace dynamics, while usage-data model improvement is a baseline necessity once many models reach near-frontier intelligence rather than a moat and incumbent compute alone did not automatically create durable AI positions, so durable AI businesses still need distribution, workflow ownership, proprietary context, customer trust, operational depth, genuine network effects if achievable, or a superior ability to build and iterate quickly.
He thinks distribution alone does not save products that are now easy for customers to rebuild and tailor themselves, and marketing cannot rescue automated factories that ship useless products nobody will use.
...
He believes a product becomes worth buying when it clears a difficulty, trust, or operational bar that makes customers prefer buying over rebuilding, and he sees a large gap between interesting research and that bar: usable context, out-of-the-box accuracy without mandatory fine-tuning, and grab-and-go packaging create a real product delta that storytelling or marketing alone cannot substitute, while academic first-mover credit is hard to settle and secondary.
...
He believes frontier labs can temporarily make subsidy itself a moat, especially when power users receive far more compute value than their subscriptions cost and consumer mindshare can spill into enterprise or API demand, but he rejects treating consumer engagement alone as the winning signal when the consumer business model is unproven and free users are heavily subsidized.
...
Evidence: https://x.com/kunchenguid/status/2103033844370903466, https://x.com/kunchenguid/status/2103145928517632234
...
He thinks good ideas start with named people who care about a real problem, not with abstract brainstorming or technology-first excitement, and for his own projects he chooses problems that are obviously useful to himself or to a few real people who have already described the pain clearly.
He rejects blaming mainstream indifference to agent building on a lack of imagination: curiosity makes early adopters, but people are not responsible for inventing problems they do not have, and for most people AI still feels like a vitamin rather than a painkiller, so the gap is Silicon Valley builders solving their own problems while product and messaging still need to find real painkiller value for ordinary consumers.
...
He worries AI creates a sharp asymmetry: innovators may get roughly 10x faster while repetition and copying get closer to 1000x easier, collapsing the old window to build traction before a copy arrives, which punishes innovators without distribution, races products that lack real network effects toward thin margins and weaker polish, and pushes choice-paralyzed users toward authorities.
...
Evidence: https://x.com/kunchenguid/status/2103655015449756131, https://x.com/kunchenguid/status/2098118138797953499
...
He does not think every company should rebuild giant SaaS products internally just because agents can write code, and he rejects the bubble claim that everyone will vibe-code their own software: weekend agent toys impress builders in a bubble, turning toys into useful products remains most of the work where current agents help least, and the mainstream remains mostly consumers of software built by others, as with blogs, Instagram, and TikTok.
...
He revised an earlier view that foreign-person gating for online model access was unenforceable after learning it fits export-control concepts such as deemed exports, and now sees the sharper problem as product and governance design: labs could build KYC-style screening if forced, but sudden emergency orders on products not built for that constraint can turn into broad shutdowns, privacy-sensitive UX, and business-model disruption.
...
He believes AI can create disastrous societal outcomes without first achieving AGI: current models still lack the judgment needed for valuable autonomous outcomes, but creating damage does not require that judgment because damage is much easier than creating value.
He thinks intentional AGI takeover that deliberately turns on humans is less probable than a weak or incompetent AI deployed with massive compute that does stupid things at scale and causes massive harm before humans can react, later explaining it as a genuine mistake.
He treats intentional human misuse of AI for large-scale harm as another real scenario, closer to terrorism, and expects prevention to come largely from making it very difficult for attackers to acquire enough compute to operate at scale.
Evidence: https://x.com/kunchenguid/status/2099354729235247546, https://x.com/kunchenguid/status/2097523254806351967
